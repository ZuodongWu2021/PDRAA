...
void FetchPP(PP &pp, std::string pp_filename)
{
 ...
 void Send_Token_Client(NetIO &io, PP &pp, std::vector<ECPoint> Token)
{
...
std::vector<uint8_t> Server(NetIO &io, PP &pp, std::vector<block> &vec_Y, BigInt sk_c)
{
    ...
    for(auto i = 0; i < pp.CLIENT_LEN; i++){ 
        vec_Fk1k2_X[i] = vec_Fk2_X[i] * sk_c; 
    }
 ...
 /*void Send_UID_Server(NetIO &io, PP &pp, std::vector<ECPoint> UID)
{

    std::vector<ECPoint> vec_Fk2_X_UID(100); 
    PrintSplitLine('-'); 
    auto start_time = std::chrono::steady_clock::now(); 
    BigInt k2 = GenRandomBigIntLessThan(order); // pick a key
     for(auto i = 0; i < pp.SERVER_LEN; i++){
        vec_Fk2_X_UID[i] = UID[i]* k2; // H(x_i)^k2
...
void Client(NetIO &io, PP &pp, std::vector<block> &vec_X,std::vector<ECPoint> UID) 
{    
    ...
    #pragma omp parallel for num_threads(NUMBER_OF_THREADS)
    for(auto i = 0; i < pp.SERVER_LEN; i++){
        vec_Fk2_X[i] = UID[i]* k2; // H(x_i)^k2
        
    } 
    for(auto i = pp.SERVER_LEN; i < pp.CLIENT_LEN; i++){
